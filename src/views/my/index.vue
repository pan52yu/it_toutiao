  ...
  <div class="my-container">
    ...
    <div class="grid-nav">
      <van-grid clickable :column-num="2">
        <van-grid-item>
          <template #icon>
            <tou-tiao icon="shoucang"></tou-tiao>
          </template>
          <template #text>
    ...
    <van-cell @click="logOut" title="退出登陆" class="logout" />
  </div>
</template>
...
export default {
  ...
  methods: {
    async logOut() {
      ...
      console.log("成功了");
      // 点击退出时 清除token vuex中已经封装好setUser可以直接调用
      this.$store.commit("setUser", null);
    },
    async getUserInfo() {
      const res = await getUserInfo();
      this.userInfo = res.data.data;
      this.userInfo.fans_count = 9999;
      console.log(res);
    },
  },
};
